...
class ToyAimmdVisualizer:
    def __init__(self, pes, temperature=1, resolution=501, descriptor_dims=[0,1], dims_extent = None):
        """
        ...
        self.x = np.linspace(pes.extent[0], pes.extent[1], resolution)
        self.y = np.linspace(pes.extent[2], pes.extent[3], resolution)
    ...
    def theoretical_q_contour(self, theoretical_committor_path, n_x, n_y=None, fig=None, ax=None):
        """
        Plot the contour of the theoretical q function.
        """
        if n_y is None:
    ...
    def plot_q_model_RPE_data(self, plot_model, n_bins_2d=100, v_min_max = None, descriptor_dims=None, fig=None, ax=None):
        ...
        im1 = ax.imshow(average_q_model, interpolation='nearest', origin='lower',cmap=self.cmap,
                extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]],alpha=0.8,aspect="auto",
                norm=norm)
        X, Y = np.meshgrid(xedges[1:], yedges[1:])
        if fig is not None:
            fig.colorbar(im1)

    def plot_q_model_RPE_data_contours(self, plot_model, n_bins_2d=100, descriptor_dims=None, fig=None, ax=None):
        average_pb_model, average_q_model, xedges, yedges = self.committor_model_2d_RPE(plot_model,n_bins_2d=n_bins_2d, descriptor_dims=descriptor_dims)
        X, Y = np.meshgrid(xedges[1:], yedges[1:])
        if ax is None:
            fig, ax = plt.subplots(1)
        contour = ax.contour(X, Y,average_q_model ,levels = self.levels_q_model, cmap=self.cmap_RPE_committor_contours, linewidths=self.linewidth_committor_contour)
    ...
    def plot_loss_allong_q_model(self, plot_model,ax=None, fig=None, density=False):
        p_B_model_RPE, q_model_RPE = self.model_output_RPE(plot_model)
        _ , weights_total, shot_results_total = self.RPE.create_total_trainset()
        loss = snapshot_loss_original(torch.tensor(q_model_RPE),torch.tensor(weights_total),torch.tensor(shot_results_total))
        print("loss original log likelihood loss: ", np.sum(loss.detach().numpy())/np.sum(weights_total))
        n_bins = 100
        if density==True:
            density_indicator = "*"
        else:
            density_indicator = ""
        prob_allong_q ,edges= np.histogram(q_model_RPE[:,0],
                                                       bins=n_bins, weights = loss/np.sum(weights_total), density=density)
        if ax is None:
            fig, ax = plt.subplots(1,1)
        ax.plot(edges[1:],prob_allong_q, label=r"$\ln \mathcal{L}(q')$ "+ "{}".format(density_indicator))
    
    def plot_loss_scaled_q_allong_q_model(self, plot_model,ax=None, fig=None):
        p_B_model_RPE, q_model_RPE = self.model_output_RPE(plot_model)
        _ , weights_total, shot_results_total = self.RPE.create_total_trainset()
        ...
        if ax is None:
            fig, ax = plt.subplots(1,1)
        ax.plot(edges[1:],prob_allong_q, label=r"$\ln \mathcal{L}(q')(1+9e^{-q'^2/5})$")
    
    def plot_loss_scaled_weight_sqrtrho(self, plot_model,ax=None, fig=None):
        p_B_model_RPE, q_model_RPE = self.model_output_RPE(plot_model)
        _ , weights_total, shot_results_total = self.RPE.create_total_trainset()
    ...
    def plot_loss_normalized_q_allong_q_model(self, plot_model,ax=None, fig=None, density=False):
        p_B_model_RPE, q_model_RPE = self.model_output_RPE(plot_model)
        _ , weights_total, shot_results_total = self.RPE.create_total_trainset()
        loss = snapshot_loss_original(torch.tensor(q_model_RPE),torch.tensor(weights_total),torch.tensor(shot_results_total))
        n_bins = 100
        ...
        scaling = torch.tensor(np.nan_to_num(1/np.sqrt(H_q[q_bin_indices]))[:,0]).float()
        print(r"loss devided by $1/rho_PE(q))$: ", np.sum((loss*scaling).detach().numpy())/np.sum(weights_total))

        if ax is None:
            fig, ax = plt.subplots(1,1)
        ax.plot(edges[1:],loss_allong_q/prob_allong_q, label=r"$\ln \mathcal{L}(q')$"+ "{}".format(density_indicator_loss)+r"$/rho^{PE}(q')$"+ "{}".format(density_indicator))

    def plot_distribution_of_points_allong_q_model(self, plot_model,ax=None, fig=None, density=False):
        p_B_model_RPE, q_model_RPE = self.model_output_RPE(plot_model)
        n_bins = 100
        if density==True:
            density_indicator = "*"
    ...
    def plot_pAandpB_of_RPE_data_allong_q_model(self, plot_model,ax=None, fig=None):
        p_B_model_RPE, q_model_RPE = self.model_output_RPE(plot_model)
        _ , weights_total, shot_results_total = self.RPE.create_total_trainset()
        n_bins = 100
        ...
        rhoB_allong_q, edges= np.histogram(q_model_RPE[:,0],
                                                       bins=n_bins, weights=weights_total*shot_results_total[:,1], density=False)
    ...
    def plot_plnp_allong_q_model(self, plot_model,ax=None, fig=None):
        p_B_model_RPE, q_model_RPE = self.model_output_RPE(plot_model)
        _ , weights_total, shot_results_total = self.RPE.create_total_trainset()
    ...
    def plot_weight_allong_q_model(self, plot_model,ax=None, fig=None, density=False):
        p_B_model_RPE, q_model_RPE = self.model_output_RPE(plot_model)
        _ , weights_total, shot_results_total = self.RPE.create_total_trainset()
        weights_total *= 1/np.sum(weights_total)
    ...
    def scatter_rc_minima_path(self, plot_model, q_bins=np.arange(-12,12,0.1), descriptor_dims=[0,1], ax=None,fig=None):
        descriptors_total, weights_total, shot_results_total = self.RPE.create_total_trainset()
        p_B_model_RPE, q_model_RPE = self.model_output_RPE(plot_model)
        ...
        ax.scatter(self.moving_average(x_value_allong_q[0,:],5),self.moving_average(x_value_allong_q[1,:],5), color="black",s=3, alpha=0.3+0.7*(bin/len(q_bins)) )
    ...
    def minimum_energy_rc_descriptors_allong_q(self, plot_model, n_descriptors=22, q_bins=np.arange(-12,12,0.1),ax=None,fig=None,plot=True):
        descriptors_total, weights_total, shot_results_total = self.RPE.create_total_trainset()
        p_B_model_RPE, q_model_RPE = self.model_output_RPE(plot_model)
        ind_bin_x = np.digitize(q_model_RPE, q_bins, right=False)
